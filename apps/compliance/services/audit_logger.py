"""
Smart Employee Management System — Statutory Compliance Audit Trail Ledger
Statutory filings, audit logs, and POSH case notes.
"""
...
class ComplianceAuditLogger:
    """
    Immutable audit trail recorder for Statutory Compliance.
    """
    ...
    def record_audit_event(
        ...
        now = datetime.now()
        raw_hash_data = f"{eid}|compliance|{action}|{actor_id}|{now.isoformat()}"
        checksum = hashlib.sha256(raw_hash_data.encode('utf-8')).hexdigest()

        return AuditLedgerEntry(
            entry_id=f"AUD-COM-{eid}",
