"""
Smart Employee Management System — Attendance Clocking Audit Trail Ledger
Biometric punch logs, geofence validations, and manual regularizations.
"""
...
class AttendanceAuditLogger:
    """
    Immutable audit trail recorder for Attendance Clocking.
    """
    ...
    def record_audit_event(
        ...
        now = datetime.now()
        raw_hash_data = f"{eid}|attendance|{action}|{actor_id}|{now.isoformat()}"
        checksum = hashlib.sha256(raw_hash_data.encode('utf-8')).hexdigest()

        return AuditLedgerEntry(
            entry_id=f"AUD-ATT-{eid}",
