"""
Smart Employee Management System — Agile Tasks Kanban Audit Trail Ledger
...
class TasksAuditLogger:
    """
    Immutable audit trail recorder for Agile Tasks Kanban.
    """
    ...
    def record_audit_event(
        ...
        now = datetime.now()
        raw_hash_data = f"{eid}|tasks|{action}|{actor_id}|{now.isoformat()}"
        checksum = hashlib.sha256(raw_hash_data.encode('utf-8')).hexdigest()

        return AuditLedgerEntry(
            entry_id=f"AUD-TAS-{eid}",
            module_name="tasks",
            action_type=action,
