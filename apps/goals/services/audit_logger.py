"""
Smart Employee Management System — OKR Goal Progress Audit Trail Ledger
...
class GoalsAuditLogger:
    """
    Immutable audit trail recorder for OKR Goal Progress.
    """
    ...
    def record_audit_event(
        ...
        now = datetime.now()
        raw_hash_data = f"{eid}|goals|{action}|{actor_id}|{now.isoformat()}"
        checksum = hashlib.sha256(raw_hash_data.encode('utf-8')).hexdigest()

        return AuditLedgerEntry(
            entry_id=f"AUD-GOA-{eid}",
            module_name="goals",
            action_type=action,
