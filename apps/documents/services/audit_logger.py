"""
Smart Employee Management System — Document Integrity Audit Trail Ledger
Uploads, signature certifications, and access logs.
"""
...
class DocumentsAuditLogger:
    """
    Immutable audit trail recorder for Document Integrity.
    """
    ...
    def record_audit_event(
        ...
        now = datetime.now()
        raw_hash_data = f"{eid}|documents|{action}|{actor_id}|{now.isoformat()}"
        checksum = hashlib.sha256(raw_hash_data.encode('utf-8')).hexdigest()

        return AuditLedgerEntry(
            entry_id=f"AUD-DOC-{eid}",
