"""
Smart Employee Management System — Payroll Execution Audit Trail Ledger
Pay run batches, salary slip releases, and bank transfer UTRs.
"""
...
class PayrollAuditLogger:
    """
    Immutable audit trail recorder for Payroll Execution.
    """
    ...
    def record_audit_event(
        ...
        now = datetime.now()
        raw_hash_data = f"{eid}|payroll|{action}|{actor_id}|{now.isoformat()}"
        checksum = hashlib.sha256(raw_hash_data.encode('utf-8')).hexdigest()

        return AuditLedgerEntry(
            entry_id=f"AUD-PAY-{eid}",
            module_name="payroll",
            action_type=action,
