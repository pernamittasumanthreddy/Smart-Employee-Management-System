"""
Smart Employee Management System — AI Analytics ML Audit Trail Ledger
...
class InsightsAuditLogger:
    """
    Immutable audit trail recorder for AI Analytics ML.
    """
    ...
    def record_audit_event(
        ...
        now = datetime.now()
        raw_hash_data = f"{eid}|insights|{action}|{actor_id}|{now.isoformat()}"
        checksum = hashlib.sha256(raw_hash_data.encode('utf-8')).hexdigest()

        return AuditLedgerEntry(
            entry_id=f"AUD-INS-{eid}",
