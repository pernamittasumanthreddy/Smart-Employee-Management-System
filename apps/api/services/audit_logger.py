"""
Smart Employee Management System — API Integration Audit Trail Ledger
API key creations, endpoint requests, and webhook deliveries.
"""
...
class ApiAuditLogger:
    """
    Immutable audit trail recorder for API Integration.
    """
    ...
    def record_audit_event(
        ...
        now = datetime.now()
        raw_hash_data = f"{eid}|api|{action}|{actor_id}|{now.isoformat()}"
        checksum = hashlib.sha256(raw_hash_data.encode('utf-8')).hexdigest()

        return AuditLedgerEntry(
            entry_id=f"AUD-API-{eid}",
            module_name="api",
            action_type=action,
